...
// ---- multer config ----
// Use Railway persistent volume if available, else local uploads dir
...
// EXCEPTION: file serving — fetched directly by EpubReader without auth header

// GET /api/books/file/:filename — serve uploaded ebook file (public, before auth)
  ...
  try {
    ...
    if (!apiKey || apiKey.startsWith('sk-your-')) {
      return res.status(400).json({
        ...
        hint: '请在 Railway 环境变量中设置 DEEPSEEK_API_KEY（获取：https://platform.deepseek.com/api_keys）'
      });
    }
...
请根据我的阅读偏好，推荐 3 本类似主题或风格的书。`
    ...
    if (err.status === 401 || err.code === 'invalid_api_key') {
      return res.status(400).json({
        ...
        hint: '请检查 Railway 环境变量中的 DEEPSEEK_API_KEY 是否正确'
      });
    }
    next(err);
  }
